...
Da $R_i$ eine Äquvivalenzsrelation ist folgt nach \Theo{aehnlichkeitstheorie} und  \Tab{attributesIncludingR} das für jedes $K_i$ gilt:
\begin{itemize}
...
Es ist zu beachten, dass jeweils der positiv wie der negativ Fall festgehalten wird \vglHuth{S. 342f}.
...
Negative Wissensaussagen der form $\neg K \phi$ können also im zeitlichen Verlauf ihre Gültigkeit verlieren \vglHuth{S. 342f}.
...
Die Lösung des \WMRs benötigt folglich zwei Schritte \vglHuth{S. 342f}.
...
\Abb{wise_men_01} zeigt den formalen Beweis des ersten Schnittes, \Abb{wise_men_02} den zweiten Schritt.


\paragraph{Beweisdiskussion \WMR Schritt 01}
\underline{Alle Linenverweise beziehen sich auf \Abb{wise_men_01}}.\\
Die \LNs{1-5} beschreiben die Vorgaben, dass Grundwissen, dass bereits auf dem Umgangssprachlichen Text extrahier wurde.\\
Zur Erinnerung: Ziel ist es die Aussage $C(p_2 \vee p_3)$ (Ausschluss der Kombination RWW) nachzuweisen.\\
Der Beweis beginnt mit der Eröffnung einer $C$ Box in \LN{6}, weil am Schluss die Aussage $p_2 \vee p_3$ ebenfalls Allgemein Wissen sein muss.\\
In \LN{7} wird begonnen die Aussage durch einen Widerspruchsbeweis zu belegen.\\
\LNs{8, 9} sind ein Beispiel dafür das die $C_e$ Regel immer angewendet werden kann, egal wie tief die Boxen geschachtelt sind.
Allgemeinwissen ist so gesehen jederzeit verfügbar.\\
Die \LNs{10-12} wurden ein wenig abgekürzt.
Strickt gesehen wir in \LN{10} mithilfe der $\wedge e$ regeln erst jeder der beiden Teile einzeln extrahiert und mit begin der $K_1$ Box in \LN{11} das $K_1$ eliminiert.
Da nun beide Aussagen $\neg p_i, i=\{2,3\}$ gelten können diese mit $\wedge i$ zusammengeführt werden.\\
In Zeile \LN{15} wir der Aussage ein $K_1$ vorangestellt, weil das Folgern aus sich das Wissen aus $K_1$ abgeschlossen ist.\\
Danach wird durch das Voranstellen des $K_1$ festgehalten das die Folgerungen nur \emph{nach allem was $K_1$ weiß} wahr sind.\\
Die \LNs{15-17} zeigen den daraus folgenden Widerspruch an was die Annahme aus \LN{7} widerlegt woraus nach anwenden der \deMorganRegeln in \LN{18} die gewünschte Aussage bewiesen ist, die, weil alles im Bereich des Allgemeinwissens geführt wurde, ebenfalls zum Allgemeinwissen zählen \underline{muss}.
...
\begin{figure}[h!t]
	...
	\end{equation*}
	\caption{Schritt 01 des three \WMRs \\ Quelle: \citeHuth{S. 345}}
	\label{fig:wise_men_01}
\end{figure}



\paragraph{Beweisdiskussion \WMR Schritt 02}
\underline{Alle Linenverweise beziehen sich auf \Abb{wise_men_02}}.\\
Dem Vorwissen wurden die Folgerung aus dem ersten Schritt hinzugefügt (\LN{6}).\\
Ziel ist es die Aussage $K_3 p_3$ nachzuweisen.
Deswegen wird in \LN{7} eine $K_3$ Box eröffnet.
Damit finden alle Folgerungen aus der Wissenssicht von $K_3$ statt.\\
Bis \LN{16} folgt der Beweis einfachen Regeln.\\
In \LN{16} formuliert beschreibt, dass, da $\neg K_2 p_2$ zum Allgemeinwissen gehört \LN{5}, jeder der Weisen $K_i$ mit $i = \{1,2,3\}$ diesen Umstand weiß.\\
...
\begin{figure}[h!t]
	...
	\end{equation*}
	\caption{Schritt 02 des three \WMRs \\ Quelle: \citeHuth{S. 346}}
